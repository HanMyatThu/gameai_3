...
class World:
  ...
  def _add_pipe(self):
    ...
    print(f"Switched to pattern: {self.current_pattern_name.upper()}, Pipe Pair: {pipe_pair}, Gap: {pipe_gap}")
  ...
  def scrollX(self):
      ...
      if self.game_over:
          ...
          self.last_milestone = 0
          bird.level = 1
          self.is_delay_active = False  # Reset delay on game over too
      score_threshold_reached = (self.score == 14 or self.score == 29)  # Check if score is just before milestone
      if score_threshold_reached and not self.is_delay_active and self.speed > -10:
          print(f"--- Activating pipe spawn delay before speed increase (Score: {self.score}) ---")
          self.is_delay_active = True  # Activate delay
      ...
      if milestone > self.last_milestone and bird.score < 45 and milestone != 0:
          ...
          if self.is_delay_active:
              print(f"--- Deactivating pipe spawn delay ---")
              self.is_delay_active = False
  ...
  def update_score_and_reward(self):
      ...
      for pipe in self.upcoming_pipes:
          ...
          if not pipe.get_is_flipped() and pipe not in self.scored_pipes and bird.rect.centerx > pipe.rect.centerx:
              self.score += 1  # <<< INCREMENT ACTUAL SCORE
              ...
              print(f"Passed Pipe! Score: {self.score}, Reward: {self.current_reward}")
  ...
  def step(self, action):
      ...
      #     print(f"switched {self.dist}")

      spawn_threshold = WIDTH - (PIPE_SIZE * self.dist)
      if self.is_delay_active:
          # Make the required gap significantly larger
          # E.g., require the rightmost pipe to be almost off-screen
          # You might need to tune this value. Lower means larger gap.
          delay_spawn_threshold = WIDTH * 0.5  # Example: require pipe < 20% from left edge
          spawn_threshold = min(spawn_threshold, delay_spawn_threshold)  # Use the stricter threshold
          # print(f"Delay Active! Spawn Threshold: {spawn_threshold:.0f}") # Debug

      if rightmost_x < spawn_threshold:
          self._add_pipe()
          print(f"Pipe spawn dist: {self.dist}")
